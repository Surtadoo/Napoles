...
export function App() {
  ...
  const [streamQuality, setStreamQuality] = useState('4K 60fps');
  ...
  // Helper to ensure stream and videoUrl are never both set
  const setCleanStreamState = (newState: Partial<StreamState>) => {
    const cleanState: Partial<StreamState> = { ...newState };
    if (cleanState.stream && cleanState.stream !== null) {
      cleanState.videoUrl = undefined;
    } else if (cleanState.videoUrl) {
      cleanState.stream = null;
    }
    setStreamState((prev) => ({ ...prev, ...cleanState }) as StreamState);
  };
    ...
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-[#0d0f12] text-gray-200 select-none">
      ...
      <div className="flex-1 flex overflow-hidden min-h-0 relative">
        ...
        <StreamView
          ...
          sharedScreens={participants.filter(p => p.isScreenSharing && p.id !== 'current-user').map(p => ({ id: p.id, name: p.name, quality: '4K 60fps', isVideo: false }))}
          onSelectSharedScreen={(screen) => {
            showToast(`Visualizando tela de ${screen.name}`);
            setStreamState({
              type: 'screen',
              stream: null,
              title: `Tela de ${screen.name}`,
      ...
      <VideoSourceModal
        ...
        onSelectVideoSource={(info) => {
          setStreamState({
            type: info.type === 'youtube' ? 'youtube' : info.type === 'twitch' ? 'youtube' : info.type === 'kick' ? 'youtube' : 'sample',
            stream: null,
            videoUrl: info.url,
            title: info.title,
            quality: streamQuality,
          ...
          addSystemMessage(`Transmitindo fonte externa: ${info.title}`);
        }}
      />
